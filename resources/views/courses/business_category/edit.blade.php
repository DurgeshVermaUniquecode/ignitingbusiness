            ...
            <div class="col-xl">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">Update Business Category</h5>
                    </div>
                    <div class="card-body">
                        <form action="{{route('edit_business_category',[$category->id])}}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-6">
                                        ...
                                        <select class="form-select" id="package" name="package"
                                            ...
                                            @foreach ($packages as $package)
                                                <option value="{{$package->id}}" @selected(old('package_id',$category->id) == $package->id)>{{$package->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @error('package')
                                    ...
                                    <div class="mb-6">
                                        ...
                                        <div class="input-group">
                                            <textarea id="description" name="description" class="form-control" placeholder="description" aria-label="description"
                                                aria-describedby="description2">{{ old('description',$category->description) }}</textarea>
                                        </div>
                                        @error('description')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
