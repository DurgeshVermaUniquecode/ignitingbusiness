    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card">
            <div class="card-header border-bottom d-flex justify-content-between">
                ...
                <a href="{{ route('add_course') }}" class="btn add-new btn-primary"><span><span
                            class="d-flex align-items-center gap-2"><i class="icon-base ti tabler-plus icon-xs"></i> <span
                                class="d-none d-sm-inline-block">Add Course</span></span></span></a>
            </div>
        ...
        function deleteCourses(courseId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    let url = "{{ route('status_course', ':id') }}".replace(':id', courseId);

                    $.ajax({
                        url: url,
                        method: 'GET',
                        success: function(response) {
                            Swal.fire('Deleted!', 'Course status changed.', 'success').then(() => {
